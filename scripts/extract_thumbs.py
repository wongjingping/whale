...
whales = pd.DataFrame({'whale':pd.unique(train['whaleID']),
	'n_total':-1,'n_label':-1})
	...
	for i in ssl.index:
		...
		for x_ in range(x_start,x+1,w2p):
			for y_ in range(y_start,y+1,h2p):
				...
				thumb_i.save(dir_i+'/'+f_'_1_'+str(x_)+'_'+str(y_)+'.jpg')
				thumb_i2 = thumb_i.transpose(Image.ROTATE_90)
				thumb_i2.save(dir_i+'/2_'+str(x_)+'_'+str(y_)+'.jpg')
				thumb_i3 = thumb_i.transpose(Image.ROTATE_180)
				thumb_i3.save(dir_i+'/3_'+str(x_)+'_'+str(y_)+'.jpg')
				thumb_i4 = thumb_i.transpose(Image.ROTATE_270)
				thumb_i4.save(dir_i+'/4_'+str(x_)+'_'+str(y_)+'.jpg')
